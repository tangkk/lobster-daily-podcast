...
- In every canonical Daily Brief spoken script, write `DRAM` as `D-RAM`, `SRAM` as `S-RAM`, `VRAM` as `V-RAM`, `NAND` as `nan`, and `Qwen` as `千问` so the intended readings are explicit before the TTS preparation layer.
